...
def a2h5_workflow(root_path:str, asoid_dir:str):
    ...
    for fp, idd in fpd.items():
        ...
        sub_array, _, _ = remap_trunc_df(sub_meta_path, sub_asoid_pred)

        new_len = min(len(dom_array), len(sub_array))
        dom_array = dom_array[0:new_len]
        sub_array = sub_array[0:new_len]

        combined_array = np.vstack((dom_array,sub_array))
        h5_path = os.path.join(os.path.dirname(idd["dom"]), f"{fp}.h5")

        save_to_h5(h5_path, combined_array, dom_behav, dom_color, fp)
...
def save_to_h5(h5_path: str, behav_array: np.ndarray, behav_dict: Dict[str, int], color_dict: dict, fpid: str, fps: int=10):
    try:
        ...
        with h5py.File(h5_path, 'w') as f:
            ...
            grp_data.create_dataset('behaviors', data=np.transpose(behav_array))
